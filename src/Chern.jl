...
"""
function FindLinks(Ham::Hamiltonian, subset::Vector{Int64})
...
"""
function FieldStrength(Links::Tuple{Matrix{ComplexF64}})

    Field   =   Links[1] .* circshift(Links[2], [-1, 0]) .* circshift(conj.(Links[1]), [0, -1]) .* conj.(Links[2])
    """
    (k+a2)*<--(Links[1])†[k+a2]---*
          |                       ^
          |                       |
...
function ChernNumber(Ham::Hamiltonian, subset::Vector{Int64})
